...
func TestEncodeGatewayDHTDiscoverResponseV2(t *testing.T) {
	...
	msg, err := EncodeGatewayDHTDiscoverResponseV2(mockContentID, mockNonce, mockFound, mockSubOffers, mockFPCs)
	assert.Empty(t, err)
...
func TestDecodeGatewayDHTDiscoverResponseV2(t *testing.T) {
	...
	contentID, nonce, found, subOffers, FPCs, err := DecodeGatewayDHTDiscoverResponseV2(validMsg)
	assert.Empty(t, err)
