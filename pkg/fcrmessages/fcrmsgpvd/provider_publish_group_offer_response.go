...
func EncodeProviderPublishGroupOfferResponse(
	...
	return fcrmessages.CreateFCRMessage(fcrmessages.ProviderPublishGroupOfferResponseType, body), nil
}

// DecodeProviderPublishGroupOfferResponse is used to get the fields from FCRMessage of ProviderPublishGroupOfferResponse
func DecodeProviderPublishGroupOfferResponse(fcrMsg *fcrmessages.FCRMessage) (
	*nodeid.NodeID, // gatewayID
	[cidoffer.CIDOfferDigestSize]byte, // digest
	error, // error
) {
	if fcrMsg.GetMessageType() != fcrmessages.ProviderPublishGroupOfferResponseType {
