...
func TestEncodeGatewayDHTDiscoverOfferResponse(t *testing.T) {
	...
	msg, err := EncodeGatewayDHTDiscoverOfferResponse(mockContentID, mockNonce, mockFound, mockSubOffers, mockFPCs)
	assert.Empty(t, err)
...
func TestDecodeGatewayDHTDiscoverOfferResponse(t *testing.T) {
	...
	contentID, nonce, found, subOffers, FPCs, err := DecodeGatewayDHTDiscoverOfferResponse(validMsg)
	assert.Empty(t, err)
