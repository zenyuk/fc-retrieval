...
func EncodeProviderPublishGroupCIDResponse(
	...
	return fcrmessages2.CreateFCRMessage(fcrmessages2.ProviderPublishGroupOfferResponseType, body), nil
}

// DecodeProviderPublishGroupOfferResponse is used to get the fields from FCRMessage of ProviderPublishGroupOfferResponse
func DecodeProviderPublishGroupOfferResponse(fcrMsg *fcrmessages2.FCRMessage) (
	*nodeid.NodeID, // gatewayID
	[cidoffer.CIDOfferDigestSize]byte, // digest
	error, // error
) {
	if fcrMsg.GetMessageType() != fcrmessages2.ProviderPublishGroupOfferResponseType {
