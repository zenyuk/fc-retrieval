...
func (o *GetProviderRegistersByIDURL) WithBasePath(bp string) *GetProviderRegistersByIDURL {
	o.SetBasePath(bp)
	return o
}
...
func (o *GetProviderRegistersByIDURL) Build() (*url.URL, error) {
	...
	var _path = "/registers/provider/{id}"

	id := o.ID
	if id != "" {
		_path = strings.Replace(_path, "{id}", id, -1)
	} else {
		return nil, errors.New("id is required on GetProviderRegistersByIDURL")
	}
...
func (o *GetProviderRegistersByIDURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetProviderRegistersByIDURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetProviderRegistersByIDURL")
	}
